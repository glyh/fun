...
let parse_ok source () =
  ignore (Parse_expand.parse_expr ~builtin_syntax source)

let parse_fail source () =
  match Parse_expand.parse_expr ~builtin_syntax source with
  | exception _ -> ()
  | _ -> Alcotest.fail "expected parse failure"

let dotted_field_shape () =
  match Parse_expand.parse_expr ~builtin_syntax "State.get" with
  | FieldAccess (Var "State", "get") -> ()
  | _ -> Alcotest.fail "expected ordinary field access"

let module_signature_sugar_shape () =
  match Parse_expand.parse_expr ~builtin_syntax "sig x : I64; y : Bool end" with
...
let module_signature_param_shape () =
  match Parse_expand.parse_expr ~builtin_syntax "fn(m : sig x : I64 end) -> m.x" with
...
let neq_still_parses_shape () =
  match Parse_expand.parse_expr ~builtin_syntax "1 != 2" with
