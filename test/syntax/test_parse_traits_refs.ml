...
let trait_decl_shape () =
  match Parse_expand.parse_expr "trait Eq A = struct eq : A -> A -> Bool end in Eq" with
...
let impl_decl_shape () =
  match Parse_expand.parse_expr "impl Eq I64 = struct let eq x y = x == y end in Eq.eq" with
...
let ref_get_shape () =
  match Parse_expand.parse_expr "!r" with
  | RefGet (Var "r") -> ()
...
let ref_set_deref_shape () =
  match Parse_expand.parse_expr "r := !r + 1" with
