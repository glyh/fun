...
let relative_path root path =
  ...
  if
    ...
    && String.equal (String.sub path 0 prefix_len) prefix
...
let test_implementation_sources_stay_under_limit () =
  ...
  let sources =
    [ "lib"; "bin" ]
    |> List.concat_map (fun dir -> collect_sources (Filename.concat root dir))
  in
  let offenders =
    sources
    |> List.filter_map (fun path ->
        let lines = count_lines path in
        if lines > max_lines then Some (relative_path root path, lines)
        else None)
  in
  ...
  | _ ->
      let details =
        ...
        |> List.map (fun (path, lines) ->
            Printf.sprintf "%s: %d lines" path lines)
        |> String.concat "\n"
      in
