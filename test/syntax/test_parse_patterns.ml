...
let nominal_type_pattern_complex_arg_shape () =
  match Parse_written.parse_expr "match T do Option(Option(I64) | I64) -> I64 | _ -> Bool end" with
...
let struct_type_pattern_open_shape () =
  match Parse_written.parse_expr "match T do struct x: p; _ end -> p | _ -> I64 end" with
...
let struct_type_pattern_closed_shape () =
  match Parse_written.parse_expr "match T do struct x: I64; y: Bool end -> I64 | _ -> Bool end" with
...
let struct_type_pattern_nominal_field_shape () =
  match Parse_written.parse_expr "match T do struct value: Option(x); _ end -> x | _ -> I64 end" with
