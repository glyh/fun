...
- Do not match `pub let` bindings or public methods in this phase, even though they may appear as visible struct members internally.
...
Update `lib/syntax/core_parser.mly` so pattern atoms accept:
...
Add syntax tests in `test/syntax/test_match_parse.ml` for:
...
```fun
let field_default : (T : Type) -> I64 = fun T ->
  match T with
  | struct x: p; _ end -> match p with I64 -> 1 | _ -> 0 end
  | _ -> 0
  end
```
...
```fun
match struct x: I64; y: Bool; end with
| struct x: I64 end -> 1
