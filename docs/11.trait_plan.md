...
impl Eq I64 = struct
  let eq x y = eq_i64 x y
end

let same : {A : Eq} -> A -> A -> Bool = fun {A} x y -> Eq.eq x y
```

Multi-trait bounds use Rust-like `+` syntax:

```fun
let encode_key : {A : Eq + Jsonable} -> A -> String = ...
```
...
- Trait operation signatures use `struct ... end` syntax.
- Implementations use `impl Trait Args = struct ... end` syntax.
...
```fun
trait Eq A = struct eq : A -> A -> Bool end in body
impl Eq I64 = struct let eq x y = eq_i64 x y end in body
let f : {A : Eq} -> A -> A -> Bool = ...
let f : {A : Eq + Jsonable} -> ... = ...
```
