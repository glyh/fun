...
and the verification rulings merged. A method's `~>` row merged. Running: [procedural macros](port-procedural-macros.md),
[trait op by argument](trait-op-takes-innermost-impl.md). Queued: prelude stage 2. The deferred cleanups (the `Syntax.AddScope` / `Syntax.Map` duplicate)
