...
type t = {
  ...
  macro_cache : (string, (string * Expand_ctx.macro_entry * Syntax.MacroKind.t * Syntax.hole_kind list) list) Hashtbl.t;
...
let parse_runtime_module t ?eval_and_apply ?syntax_nominals path =
  ...
  let load_macros ctx _path =
    ...
    | Some macros ->
        List.iter (fun (name, entry, kind, params) ->
          Hashtbl.replace ctx.Expand_ctx.macro_table name entry;
       ...
       (* Cache macros from expansion context so elaborator can find them *)
        Hashtbl.iter (fun name entry ->
          ...
          Hashtbl.replace t.macro_cache resolved
            ((name, entry, kind,
              Option.value ~default:[] (Expand_ctx.lookup_macro_params ctx name)) :: (Option.value ~default:[] (Hashtbl.find_opt t.macro_cache resolved))))
