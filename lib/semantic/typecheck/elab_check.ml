...
let check ops (ctx : Ctx.t) (expr : Syntax.t) (expected : value) : term =
  ...
  | Lam _, _ when Elab_poly_arrows.lambda_has_poly ~alias:(Elab_type_expr.poly_row_alias ctx) expr ->
      ops.check ctx (Elab_poly_arrows.lambda ~alias:(Elab_type_expr.poly_row_alias ctx) expr) expected
