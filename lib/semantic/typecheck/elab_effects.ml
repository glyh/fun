...
let require_handled_at_entry ~since ctx effects =
  check_effect_subset ctx effects (runtime_handled_effects ctx);
  (* A written [->{_}] nothing solved: an error, never a default (effects
     follow-ups, item 4). A meta restored away by a failed trial is gone. Only
     this entry's rows count: [since] is the first meta it created. *)
  let metas = ctx.Ctx.metas in
  Dynarray.iter
    (fun id ->
      if since <= id && id < MetaContext.count metas then
