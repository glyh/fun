...
let parse_expr_with_ctx ?elaborate ?eval_and_apply ?load_macros ?load_syntax ?open_prelude ?syntax_nominals ?context_kind source =
  let context_kind = match context_kind with Some k -> k | None -> Syntax.MacroKind.(Expr (None, None)) in
  Enforest.parse_expr ?load_syntax ?open_prelude source |> expand_lower_syntax ?elaborate ?eval_and_apply ?load_macros ?syntax_nominals ~context_kind

let parse_expr ?elaborate ?eval_and_apply ?load_macros ?load_syntax ?open_prelude ?syntax_nominals ?context_kind source =
